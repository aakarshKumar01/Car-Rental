...
const ManageCars = () => {
  ...
  const deleteCar = async(carId) => {
    try {
      ...
      const confiem = window.confirm('Are you sure you want to delete this car?')
  ...
  useEffect(() => {
    isOwner && fetchOnerCars()
  },[isOwner])

  return (
    <div className='px-4 pt-10 md:px-10 w-full'>
      <Title title="Manage Cars" subTitle="View all listed cars, update their details, or remove them from the bookings platfor," />
        ...
        <tbody>
          {
            cars.map((car, index) => (
              <tr key={index} className='border-t border-borderColor'>
                ...
                <td className='p-3 max-m:hidden'>{car.category}</td>
                <td className='p-3 max-m:hidden'>{currency}{car.pricePerDay}/day</td>
                <td className='p-3 max-m:hidden'>
